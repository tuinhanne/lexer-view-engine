...
| `componentPaths` | `string[]` | `[]` | Narrower subset scanned first for component lookup. Falls back to `viewPaths` when empty. |
...
| `componentPath(string $dir)` | Add a directory for auto-discovered components |
